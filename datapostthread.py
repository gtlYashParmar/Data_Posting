...
def upload_recordings_to_server(candidate_id, dest_id, recordings):
    ...
    for idx, recording_file in enumerate(recordings, 1):
        ...
        files.append(('recording_files', (recording_filename,
                     open(recording_file, 'rb'), 'audio/mpeg')))

    response = requests.put(f"{LOCAL_URL}/{dest_id}/",
    ...
    if response.status_code == 200:
        ...
        logging.error(
            f"Failed to upload files for candidate {dest_id}. Status code: {response.status_code}")


def process_candidate(candidate_data):
    try:
        source_id, destination_id = candidate_data
...
def worker():
    while True:
        candidate_data = candidate_queue.get()
...
def main():
    ...
    for candidate_data in tqdm(all_candidate_data.items()):
        candidate_queue.put(candidate_data)
